...
Shopify.addItemFromForm = function(form, callback, errorCallback) {
  ...
  var params = {
    ...
    error: function(XMLHttpRequest, textStatus) {
      $('#AddToCartText-product-template').html('Sold Out');
...
var ajaxifyShopify = (function(module, $) {
  ...
  modalSetup = function () {
    ...
    $modalContainer.prepend('<button class="ajaxifyCart--close" title="' + "translation missing: en.cart.general.close_cart" + '">' + "Cancel" + '</button>');
 ...
 // Create a close drawer button
    $drawerContainer.prepend('<button class="ajaxifyCart--close" title="' + "Cancel" + '">' + "Cancel" + '</button>');
    $closeCart = $('.ajaxifyCart--close');
    $closeCart.on('click', hideDrawer);
    
 	$drawerContainer.addClass('drawer_crt');
  ...
  showDrawer = function (toggle) {
    ...
    // Show the drawer
   // $('body').addClass('add_crt');
    setTimeout(function(){
       $('body').find('#dim_bg').fadeIn();
    }, 500);
    $drawerContainer.addClass('is-visible');
	//$('#PageContainer').addClass('is-visible');
    //scrollTop();
  ...
  buildCart = function (cart) {
    ...
    // Show empty cart
    if (cart.item_count === 0) {
      $cartContainer.append('<div class="cart_emty_note"><h2>' + "Your cart is currently empty." + '</h2><a href="/collections/all"><span class="cart--continue-message">' + "Continue shopping" + '</span></a></div>');
      ...
      */
      ...
      items.push(item);
          Handlebars.registerHelper('list', function(context, options) {
            var ret = '<select class="cart__qty-input" name="updates[]" id="updates_' + cartItem.variant_id + '">';

  for(var i=1, j=context; i<j; i++) {
    var $isselect = (i==options)?"selected":'';
    ret = ret + "<option rel="+options+" value="+i+" "+$isselect+">"+i+"</option>";
  }
  ...
  return ret + "</select>";
    ...
    data = {
      ...
      //totalPrice: (cart.total_price, settings.moneyFormat ),
      btnClass: $btnClass
    }
    ...
    switch (settings.method) {
      ...
      var newCurrency = $(".currency-picker").val();
      //alert(newCurrency);
      Currency.convertAll('USD', newCurrency);
...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
     _getProductSwatches: function(variant) {
      if (variant) {

        for (var i=0,length=variant.options.length; i<length; i++) {
          $('.product-form').each(function() {
            var radioButton = $(this).find('.swatch[data-option-index="' + i + '"] :radio[value="' + variant.options[i] +'"]');
            if (radioButton.size()) {
    ...
    _onSelectChange: function() {
      ...
      this._getProductSwatches = variant;
...
$(document).ready(function() {
  ...
  sections.register('product-template-2', theme.Product);
 ...
 $(document).bind('touchstart', function(event) { 
   ...
   if (!$clicked.parents().hasClass("js-mobile-nav-toggle") && !$clicked.parents().hasClass("mobile-nav-wrapper")){
     ...
     if($('#mobile_menu').is(":visible")) {
       ...
       $('.js-mobile-nav-toggle').addClass('mobile-nav--open').removeClass('mobile-nav--close');
     }
     
   }        
});

var cart_form = $('.product-single__meta').html();
$(".product_form_sticky").append(cart_form );
$('.product_form_sticky').hide();
$('.product_form_sticky').find('.single-option-selector').eq(0).attr('id','sticky_select-option-0');
$('.product_form_sticky').find('.single-option-selector').eq(1).attr('id','sticky_select-option-1');
$('.product_form_sticky').find('.single-option-selector').eq(2).attr('id','sticky_select-option-2');
$('.product_form_sticky').find('#ProductSelect-product-template-2').attr('id','select_product-2');
$('.product_form_sticky').find('#ProductSelect-product-template').attr('id','select_product-2');
 $(document).scroll(function () {
   var y = $(this).scrollTop();

   if (y > 300) {
     $('.product_form_sticky').addClass('sticky_bar');
     $('.product_form_sticky').find('.AddToCart-product-template-2').addClass('stiky_button');
	 $('.product_form_sticky').find('.AddToCart-product-template').addClass('stiky_button');
     $('.product_form_sticky').find('.stiky_button').attr('id','sticky_addtocart');
    // $('#ProductSection-product-template-2').find('.product-single__meta').hide();
//      $('.product_form_sticky').show();
     var $select1 = $( '#sticky_select-option-0' ),
         $select2 = $( '#SingleOptionSelector-0' ),
         $options = $select2.find( 'option' );
     $select1.on( 'change', function() {
       var optionindex = $('option:selected',this).index();
       $select2.prop('selectedIndex', optionindex).trigger('change');
       //alert($('.product_form_sticky').find('#ProductSelect-product-template-2').val());
      
      
       
       var mainoptionindex = $('#ProductSection-product-template-2').find('.product-single').find('.product-form').find('#ProductSelect-product-template-2 option:selected').index();
       $('.product_form_sticky').find('#select_product-2').prop('selectedIndex', mainoptionindex).trigger('change');
	   
        var mainoption_index1 = $('#ProductSection-product-template').find('.product-single').find('.product-form').find('#ProductSelect-product-template option:selected').index();
      $('.product_form_sticky.prod_sticky').find('#select_product-2').prop('selectedIndex', mainoption_index1).trigger('change');
 
	   
     } );
  

     var $select11 = $( '#sticky_select-option-1' ),
         $select12 = $( '#SingleOptionSelector-1' ),
         $options1 = $select12.find( 'option' );
     $select11.on( 'change', function() {
       var optionindex2 = $('option:selected',this).index();
       $select12.prop('selectedIndex', optionindex2).trigger('change');
       //alert($('.product_form_sticky').find('.product-single').find('.product-form').find('#ProductSelect-product-template-2').val());
    
       var mainoptionindex2 = $('#ProductSection-product-template-2').find('.product-single').find('.product-form').find('#ProductSelect-product-template-2 option:selected').index();
       $('.product_form_sticky').find('#select_product-2').prop('selectedIndex', mainoptionindex2).trigger('change');

        var mainoption_index2 = $('#ProductSection-product-template').find('.product-single').find('.product-form').find('#ProductSelect-product-template option:selected').index();
      $('.product_form_sticky.prod_sticky').find('#select_product-2').prop('selectedIndex', mainoption_index2).trigger('change');
 
       
     } );
     var $select13 = $( '#sticky_select-option-2' ),
         $select14 = $( '#SingleOptionSelector-2' ),
         $options1 = $select12.find( 'option' );
     $select13.on( 'change', function() {
       var optionindex24 = $('option:selected',this).index();
       $select14.prop('selectedIndex', optionindex24).trigger('change');
       //alert($('.product_form_sticky').find('.product-single').find('.product-form').find('#ProductSelect-product-template-2').val());
 
       var mainoptionindex25 = $('#ProductSection-product-template-2').find('.product-single').find('.product-form').find('#ProductSelect-product-template-2 option:selected').index();
       $('.product_form_sticky').find('#select_product-2').prop('selectedIndex', mainoptionindex25).trigger('change');
 
       var mainoption_index3 = $('#ProductSection-product-template').find('.product-single').find('.product-form').find('#ProductSelect-product-template option:selected').index();
      $('.product_form_sticky.prod_sticky').find('#select_product-2').prop('selectedIndex', mainoption_index3).trigger('change');
 
       
     } );
   }
   else {
     $('.product_form_sticky').removeClass('sticky_bar');
     $('.product_form_sticky').find('.AddToCart-product-template-2').removeClass('stiky_button');
	  $('.product_form_sticky').find('.AddToCart-product-template').removeClass('stiky_button');
//      $('.product_form_sticky').hide();
     //$('#ProductSection-product-template-2').find('.product-single__meta').show();
   }
   
  
 });
 $("#sticky_addtocart").on("click",function(){ 
    $('#ProductSection-product-template').find('.product-single').find('.product-form').find('#AddToCart-product-template').trigger("click");
 $('#ProductSection-product-template-2').find('.product-single').find('.product-form').find('#AddToCart-product-template-2').trigger("click");

  });
  
  $('.site-nav').find('.site-nav__dropdown').find('.hover-collection').hover(function(event){
      var collcsn = $(this).data('collsnlink');
      $(this).closest('.site-nav__dropdown').find('.Product_collection').each(function(){
      var procollescn =  $(this).data('collsnlink');      
      if(collcsn == procollescn){
        $(this).css('display','block');

    // add not filtering class to rest of buttons
    $(this).siblings('.Product_collection').css('display','none');
